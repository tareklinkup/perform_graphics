...
class Customer extends CI_Controller
{
    ...
    function customer_due_report()
    {
        ...
        $data['content'] = $this->load->view('Administrator/sales/customer_due_report', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getCustomerDueReport()
    {
        ...
        $cusClause = '';
        if(isset($data->customerId) && $data->customerId != null){
            $cusClause = " and c.Customer_SlNo = '$data->customerId' ";
        }

        $customers = $this->db->query("SELECT c.Customer_SlNo,c.Customer_Name,c.Customer_Mobile
            FROM tbl_customer c
            WHERE c.status = 'a'
            and c.Customer_Type <> 'G'
        ...
        $result = $this->db->query("SELECT date, sum(due) as due, sum(returned + paid_out) as paid from
        (select 
                        'a' as sequence,
                    ...
                    where sm.SalseCustomer_IDNo = '$value->Customer_SlNo'
                    ...
                    and cp.CPayment_customerID = '$value->Customer_SlNo'
                    ...
                    where smr.SalseCustomer_IDNo = '$value->Customer_SlNo'
                    and sr.SaleReturn_ReturnDate between '$data->fromDate' and '$data->toDate'
                    ) as tbl
                    group by MONTH(date)")->result();

                    foreach($result as $key => $item){
                        $item->month_name = date('F', strtotime($item->date));
                    }

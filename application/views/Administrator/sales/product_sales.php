	...
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			<div class="form-group">
				<label class="col-xs-1 control-label no-padding-right"> Invoice no </label>
				<div class="col-xs-2">
			...
			<div class="form-group">
				<label class="col-xs-1 control-label no-padding-right"> Sales By </label>
				<div class="col-xs-2">
			...
			<div class="form-group">
				<label class="col-xs-1 control-label no-padding-right"> Sales From </label>
				<div class="col-xs-2">
					...
					<div class="row">
						...
						<div class="col-xs-5">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group" :style="{display: selectedProduct.Unit_Name == 'PCS' || selectedBranch.brunch_id == '3' ? 'none' : ''}">
									<label class="col-xs-3 control-label no-padding-right"> Size </label>
									<div class="col-xs-5">
										<input type="number" step="0.01" id="size_w" class="form-control" ref="size_w" v-model="selectedProduct.size_w" v-on:input="sizeTotal" autocomplete="off" />
									</div>
									<div class="col-xs-4">
										<input type="number" step="0.01" id="size_h" class="form-control" ref="size_h" v-model="selectedProduct.size_h" v-on:input="sizeTotal" autocomplete="off" />
									</div>
								</div>
								<div class="form-group" :style="{display: selectedProduct.Unit_Name != 'PCS' ? '' : 'none'}">
									<label class="col-xs-3 control-label no-padding-right"> Color </label>
									<div class="col-xs-9">
										<input type="number" step="0.01" min='0' placeholder="color" id="color" class="form-control" v-model="selectedProduct.color" v-on:input="sizeTotal" autocomplete="off" :required="selectedProduct.Unit_Name != 'PCS'" />
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group" style="display:none;" :style="{display: sales.isService == 'true' && selectedBranch.brunch_id == 3 ? '' : 'none'}">
									<label class="col-xs-3 control-label no-padding-right">Description</label>
									<div class="col-xs-9">
										<textarea cols="30" rows="2" placeholder="Description..." class="form-control" v-model="selectedProduct.description"></textarea>
									</div>
								</div>
						...
						<div class="col-xs-2">
							<div v-bind:style="{display:sales.isService == 'true' ? 'none' : ''}">
								<div class="text-center">{{ productStockText }}</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td v-if="product.size_w != '' && product.size_h != ''">{{ product.size_w }} x {{ product.size_h }}</td>
							<td v-else>-</td>
							<td v-if="product.color == 0 || product.color == ''">-</td>
							<td v-else>{{ product.color }}</td>
								...
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-xs-12">
													<input type="number" id="vat" :readonly="!sales.isService" class="form-control" v-model="vatPercent" v-on:input="calculateTotal" />
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												<div class="col-xs-6">
													<input type="button" class="btn btn-default btn-xs" value="Sale" v-on:click="saveSales" v-bind:disabled="saleOnProgress ? true : false" style="color: black!important;margin-top: 0px;width:100%;padding:5px;font-weight:bold;">
												</div>
												<div class="col-xs-6">
													<a class="btn btn-info btn-xs" v-bind:href="`/sales/${sales.isService == 'true' ? 'service' : 'product'}`" style="color: black!important;margin-top: 0px;width:100%;padding:5px;font-weight:bold;">New Sale</a>
												</div>
											</div>
										</td>
									</tr>
...
<script>
	...
	new Vue({
		...
		async created() {
			...
			if (this.selectedBranch.brunch_id != 3) {
				if (this.selectedProduct.Unit_Name != "PCS") {
		...
		methods: {
			...
			sizeTotal() {
				if (this.selectedBranch.brunch_id != 3) {
					this.selectedProduct.quantity = parseFloat(parseFloat(this.selectedProduct.size_w) * parseFloat(this.selectedProduct.size_h) * parseFloat(this.selectedProduct.color)).toFixed(2);
				} else {
					this.selectedProduct.quantity = parseFloat(this.selectedProduct.quantity);
				}
				this.productTotal();
			},
			productTotal(event) {
				document.querySelector("#color").removeAttribute("readonly")
				if (this.selectedBranch.brunch_id != 3) {
					...
					if (this.selectedProduct.Unit_Name.toLowerCase() == 'plate') {
						this.selectedProduct.total = (this.selectedProduct.quantity * this.selectedProduct.color * parseFloat(this.selectedProduct.Product_SellingPrice)).toFixed(2);
					} else if (this.selectedProduct.Unit_Name.toLowerCase() == 'printing') {
						this.selectedProduct.total = (this.selectedProduct.color * parseFloat(this.selectedProduct.Product_SellingPrice)).toFixed(2);
					} else {
						this.selectedProduct.total = (parseFloat(this.selectedProduct.quantity) * parseFloat(this.selectedProduct.Product_SellingPrice)).toFixed(2);
					}
				}
			...
			async productOnChange() {
				...
				if(this.selectedBranch.brunch_id == 3){
					document.querySelector("#color").setAttribute("readonly", true)
				}
			...
			addToCart() {
				...
				let product = {
					...
					size_w: this.selectedProduct.size_w ?? '',
					size_h: this.selectedProduct.size_h ?? '',
				...
				if (this.selectedProduct.Unit_Name != 'PCS' && product.color == 0 || product.color == '') {
					alert('Enter color');
					return;
				}
				...
				let cartInd = this.cart.findIndex(p => p.productId == product.productId && p.size_w == product.size_w && p.size_h == product.size_h && p.color == product.color);
				if (cartInd > -1) {
				...
				await axios.post('/get_sales', {
					...
					r.saleDetails.forEach(product => {
						let cartProduct = {
							...
							size_w: product.SaleDetails_size_w,
							size_h: product.SaleDetails_size_h,
							color: product.SaleDetails_color,
							quantity: product.SaleDetails_TotalQuantity,
							total: product.SaleDetails_TotalAmount,
							purchaseRate: product.Purchase_Rate,
							description: product.SaleDetails_Description,
						}
